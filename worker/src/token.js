...
export function isValidGroupKey(groupKey) {
  return typeof groupKey === 'string' && /^[a-z0-9][a-z0-9_-]{0,39}$/i.test(groupKey);
}
...
export async function mintMemberTokenSigned({ groupKey, serviceAccount }) {
  if (!isValidGroupKey(groupKey)) throw new Error('INVALID_GROUP_KEY');
